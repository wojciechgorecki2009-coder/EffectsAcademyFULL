...
async def ai_usage_for_user(user: dict) -> dict:
    ...
    limit = None if unlimited else base_limit
...
async def edit_ai_image(
    ...
    if not usage.get("unlimited") and usage["remaining"] <= 0:
        raise HTTPException(
            status_code=429,
            detail=f"Monthly AI image limit reached. You can generate {usage['base_limit']} image edits per month. Upgrade to Premium for 30 monthly generations.",
        )
